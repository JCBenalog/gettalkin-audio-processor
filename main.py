...
class AudioProcessor:
    ...
    def standardize_script_formatting(self, file_content: str) -> str:
        """Standardize script formatting to ensure consistent parsing"""
        
        # Normalize all Unicode quote variants to straight quotes
        file_content = file_content.replace('"', '"').replace('"', '"').replace('"', '"')
        
        # Fix nested double quotes - convert inner quotes to single quotes
        # Pattern: ,"text "inner" text" -> ,"text 'inner' text"
        import re
        file_content = re.sub(r',"([^"]*)"([^"]*)"([^"]*)"', r',"\1\'\2\'\3"', file_content)
    ...
    def call_elevenlabs_api(self, text: str, voice_config: Dict) -> bytes:
        ...
        headers = {
            ...
            "xi-api-key": ELEVENLABS_API_KEY  # Uses secure environment variable
        }
...
def health_check():
    ...
    try:
        ...
        return jsonify({
            ...
            "message": "🔐 Secure GetTalkin Audio Processor running"
        })
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
...
if __name__ == '__main__':
    try:
        logger.info("🚀 Starting GetTalkin Audio Processor (Secure Version)")
